...
const clearTodayDealsBtn = document.getElementById("clearTodayDeals");
// ===== STEP-A: Enable biometric hold mode (visual only) =====
if (clearTodayDealsBtn) {
  clearTodayDealsBtn.classList.add("hold-mode");
}
// ===== STEP-B: Biometric press & hold logic =====
let holdTimer = null;
let isHolding = false;
...
if (clearTodayDealsBtn) {
  ...
  clearTodayDealsBtn.addEventListener("click", e => {
    e.preventDefault();
  });
  ...
  const startHold = () => {
    ...
    clearTodayDealsBtn.classList.add("holding");

    holdTimer = setTimeout(() => {
      isHolding = false;
      clearTodayDealsBtn.classList.remove("holding");

    // 📳 MOBILE VIBRATION (premium feedback)
  if (navigator.vibrate) {
    navigator.vibrate(40); // 40ms short haptic
  }

      // ✅ HOLD SUCCESS → clear deals
      clearTodayDeals();
    }, 2000);
  };
  ...
  clearTodayDealsBtn.addEventListener("pointerdown", startHold);
  clearTodayDealsBtn.addEventListener("pointerup", cancelHold);
  clearTodayDealsBtn.addEventListener("pointerleave", cancelHold);
  clearTodayDealsBtn.addEventListener("pointercancel", cancelHold);
}
...
function clearTodayDeals() {
   ...
   // ❌ user cancelled → kuch bhi clear nahi hoga
...
function updateLedgerSummary() {
  ...
  ledgerTotalEl.textContent = Number(total).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
}
...
function renderLedgerEntries() {
  ...
  const isEditingUsdt = window.__editingState && window.__editingState.type === "usdt" ? String(window.__editingState.id) : null;
  ...
  entries.forEach((entry, index) => {
    ...
    if (isEditingUsdt && String(entry.id) === isEditingUsdt) {
      html += `<tr class="editing-row" data-id="${entry.id}">
<td>${index + 1}</td>
<td>${esc(safePerson)}</td>
<td><input type="number" class="edit-amount" value="${formattedAmount || ""}"></td>
<td><input type="number" class="edit-rate" value="${formattedRate || ""}" placeholder="Optional"></td>
<td><input type="text" class="edit-notes" value="${esc(safeNotes)}" placeholder="Optional"></td>
<td><input type="date" class="edit-date" value="${esc(safeDate)}"></td>
<td>
  <button class="save-deal-btn show ledger-save-edit-btn" data-id="${entry.id}" type="button">Save</button>
  <button class="delete-deal-btn ledger-cancel-edit-btn" data-id="${entry.id}" type="button">Cancel</button>
</td>
</tr>`;
    } else {
...
<td>${esc(safeDate)}</td>
<td>
  <button class="save-deal-btn show ledger-edit-btn" data-id="${entry.id}" type="button">Edit</button>
  <button class="delete-deal-btn ledger-delete-btn" data-id="${entry.id}" type="button">Delete</button>
</td>
</tr>`;
    }
  });
...
if (ledgerEntriesBody) {
  ledgerEntriesBody.addEventListener("click", event => {
    ...
    if (editBtn) {
      ...
      if (id) enterEditMode("usdt", id);
      return;
    }
    const saveBtn = event.target.closest(".ledger-save-edit-btn");
    if (saveBtn) {
      const id = saveBtn.getAttribute("data-id");
      if (id) saveEditedEntry("usdt", id);
      return;
    }
    const cancelBtn = event.target.closest(".ledger-cancel-edit-btn");
    if (cancelBtn) {
      cancelEdit("usdt");
      return;
    }
    const delBtn = event.target.closest(".ledger-delete-btn");
    if (!delBtn) return;
...
function updatePkrLedgerSummary() {
  ...
  pkrLedgerTotalEl.textContent = Number(total).toLocaleString("en-US", { maximumFractionDigits: 0 });
}
...
function renderPkrLedgerEntries() {
  ...
  const isEditingPkr = window.__editingState && window.__editingState.type === "pkr" ? String(window.__editingState.id) : null;
  const esc = s => String(s || "").replace(/</g, "&lt;").replace(/"/g, "&quot;");
  let html = "";
  entries.forEach((entry, index) => {
    const amount = Number(entry.amount) || 0;
    ...
    const safeNotes = entry.notes || "";
    const safeDate = entry.date || "";
    if (isEditingPkr && String(entry.id) === isEditingPkr) {
      html += `<tr class="editing-row" data-id="${entry.id}">
<td>${index + 1}</td>
<td>${esc(safePerson)}</td>
<td><input type="number" class="edit-amount" value="${formattedAmount || ""}"></td>
...
<td><input type="date" class="edit-date" value="${esc(safeDate)}"></td>
<td>
  <button class="save-deal-btn show pkr-ledger-save-edit-btn ledger-save-edit-btn" data-id="${entry.id}" type="button">Save</button>
  <button class="delete-deal-btn pkr-ledger-cancel-edit-btn ledger-cancel-edit-btn" data-id="${entry.id}" type="button">Cancel</button>
</td>
</tr>`;
    } else {
      html += `<tr data-id="${entry.id}">
...
<td>${esc(safeDate)}</td>
<td>
  <button class="save-deal-btn show pkr-ledger-edit-btn ledger-edit-btn" data-id="${entry.id}" type="button">Edit</button>
  <button class="delete-deal-btn pkr-ledger-delete-btn ledger-delete-btn" data-id="${entry.id}" type="button">Delete</button>
</td>
</tr>`;
    }
  });
...
if (pkrLedgerEntriesBody) {
  pkrLedgerEntriesBody.addEventListener("click", event => {
    const editBtn = event.target.closest(".pkr-ledger-edit-btn");
    if (editBtn) {
      const id = editBtn.getAttribute("data-id");
      if (id) enterEditMode("pkr", id);
      return;
    }
    const saveBtn = event.target.closest(".pkr-ledger-save-edit-btn");
    if (saveBtn) {
      const id = saveBtn.getAttribute("data-id");
      if (id) saveEditedEntry("pkr", id);
      return;
    }
    const cancelBtn = event.target.closest(".pkr-ledger-cancel-edit-btn");
    if (cancelBtn) {
      cancelEdit("pkr");
      return;
    }
    const delBtn = event.target.closest(".pkr-ledger-delete-btn");
    if (!delBtn) return;
    const id = delBtn.getAttribute("data-id");
    if (!id) return;
    const entries = getPkrLedgerEntries();
...
/* ===== INLINE EDIT HELPERS (USDT & PKR Ledgers) ===== */
window.__editingState = null;
function enterEditMode(type, id) {
  window.__editingState = { type, id: String(id) };
  if (type === "usdt") {
    renderLedgerEntries();
  } else if (type === "pkr") {
    renderPkrLedgerEntries();
...
function saveEditedEntry(type, id) {
  const targetId = String(id);
  if (type === "usdt") {
    if (!ledgerEntriesBody) return;
    const row = ledgerEntriesBody.querySelector(`tr[data-id="${targetId}"]`);
    if (!row) return;
    const amountRaw = row.querySelector(".edit-amount")?.value || "";
    const rateRaw = row.querySelector(".edit-rate")?.value || "";
    const notes = (row.querySelector(".edit-notes")?.value || "").trim();
    const dateVal = row.querySelector(".edit-date")?.value || "";
    const amount = Number(amountRaw);
    if (!amount || !isFinite(amount)) {
      alert("USDT Amount must be a valid number.");
      return;
    }
    const rateParsed = rateRaw === "" ? "" : Number(rateRaw);
    const entries = getLedgerEntries();
    ...
    entries[idx] = {
      ...
      mode: current.mode,
      amount: amount,
      rate: rateRaw === "" || !isFinite(rateParsed) ? "" : rateParsed,
      notes: notes,
      date: dateVal || current.date || new Date().toISOString().slice(0, 10)
    };
    saveLedgerEntries(entries);
    window.__editingState = null;
    renderLedgerView();
    ...
    if (!pkrLedgerEntriesBody) return;
    const row = pkrLedgerEntriesBody.querySelector(`tr[data-id="${targetId}"]`);
    if (!row) return;
    const amountRaw = row.querySelector(".edit-amount")?.value || "";
    const rateRaw = row.querySelector(".edit-rate")?.value || "";
    const notes = (row.querySelector(".edit-notes")?.value || "").trim();
    ...
    const amount = Number(amountRaw);
    if (!amount || !isFinite(amount)) {
      alert("PKR Amount must be a valid number.");
      return;
    }
    const rateParsed = rateRaw === "" ? "" : Number(rateRaw);
    const entries = getPkrLedgerEntries();
